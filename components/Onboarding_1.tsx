...
const EDITIONS = [
  ...
  },
  {
    ...
    price: '$49/mo',
    icon: 'fa-seedling',
    features: ['Cloud Profiles', 'Basic Inventory AI', 'Standard Coach', 'Staff Monitoring'],
  ...
  },
  {
    ...
    price: '$199/mo',
    icon: 'fa-crown',
    features: ['Predictive Analytics', 'Signature Lab', 'Guest Journey AI', 'Vision Audits'],
  ...
  },
  {
    ...
    features: ['Private Silos', 'Custom Model Tuning', 'Admin User Controls', 'Priority Handshake'],
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete, onSelectAuth }) => {
  ...
  const handleNext = () => {
    ...
    // ARCHITECT GATE: Only Enterprise users (Architect) get to configure a Private Silo manually
    if (step === 3) {
      if (isDemoMode) {
        setStep(5);
        return;
      ...
      if (!isArchitectTier) {
        // Non-enterprise tiers use Managed Cloud and skip the manual entry step
        setStep(5);
        return;
      }
      if (isProductionMode || isLocalSiloBypass) {
        // Even for Architect, if we have env vars, we can skip manual entry
        setStep(5);
        return;
      }
    }
  ...
  const handleBack = () => {
    if (step === 5) {
      if (isDemoMode || !isArchitectTier || isProductionMode || isLocalSiloBypass) {
        setStep(3);
        return;
      }
    }
      ...
      case 0: 
        return (
          <div className="w-full min-h-screen bg-stone-950 text-stone-100 font-sans selection:bg-amber-500/30 selection:text-white overflow-x-hidden">
            <nav className="fixed top-0 left-0 right-0 z-[120] px-8 py-6 flex justify-between items-center bg-gradient-to-b from-stone-950 to-transparent backdrop-blur-sm lg:backdrop-blur-none">
              ...
              <div className="flex gap-4">
                ...
                {(!isProductionMode || showDevTools) && (
                  <button onClick={() => setStep(1)} className="px-6 py-2.5 bg-white text-stone-950 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-amber-50 transition-all shadow-xl">Deploy System</button>
                )}
              </div>
            </nav>
            ...
            <section id="protocols" className="py-40 px-8">
               <div className="max-w-6xl mx-auto space-y-20">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {EDITIONS.map((ed) => (
                      <div key={ed.id} className="bg-white/5 backdrop-blur-sm p-10 rounded-[3rem] border border-white/5 flex flex-col space-y-10 group hover:border-amber-500/30 transition-all hover:bg-white/[0.07] shadow-2xl relative overflow-hidden">
                         <div className="flex justify-between items-start">
                            ...
                            <div className="flex flex-col items-end">
                               ...
                               <span className="text-xl font-serif font-bold italic text-amber-500">{ed.price}</span>
                            </div>
                         </div>
        ...
        return (
          <div className="w-full max-w-6xl space-y-12 animate-in slide-in-from-right-10 duration-500 px-6">
            ...
            <div className="flex justify-between pt-10">
              ...
              <button 
                onClick={handleNext}
                className="px-14 py-5 bg-amber-600 hover:bg-amber-50 text-white rounded-2xl font-black text-xs uppercase tracking-[0.4em] transition-all shadow-2xl"
              >
                Proceed to Setup
              </button>
            </div>
        ...
        return (
          <div className="w-full max-w-3xl space-y-10 animate-in slide-in-from-right-10 duration-500 px-6">
            ...
            <div className="flex justify-between pt-16">
              ...
              >
                {isArchitectTier ? 'Proceed to Silo Setup' : 'Proceed to Tuning'}
              </button>
            </div>
          </div>
        );
        ...
        return (
          <div className="w-full max-w-2xl space-y-10 animate-in slide-in-from-right-10 duration-500 px-6">
             <div className="space-y-2">
              ...
              <p className="text-stone-500 text-lg italic">"Architect-tier protocols detected: Provisioning private data layer."</p>
            </div>
              ...
              <div className="p-8 bg-blue-600/10 rounded-[2.5rem] border-2 border-blue-500/20 flex gap-6 items-center shadow-inner">
                 ...
                 <p className="text-sm text-blue-100 font-medium italic leading-relaxed">
                   {dbStatus.isEnvManaged 
                     ...
                     : "As an Architect-level node, you can connect Vinea to your private Supabase instance for complete data sovereignty."}
                 </p>
              </div>
          ...
          <div className="w-full max-w-3xl space-y-12 animate-in slide-in-from-right-10 duration-500 px-6">
             <div className="space-y-2">
              ...
              <p className="text-stone-500 text-lg italic">
                {isLocalSiloBypass ? "Local environment identified. Synchronizing local persona." : (isArchitectTier ? "Configure your private silo persona." : "Initializing managed intelligence node...")}
              </p>
             </div>
             {isLocalSiloBypass && (
                  ...
                  <p className="text-stone-400 text-lg font-medium italic text-center">
                    {isDemoMode 
                      ...
                      : (isArchitectTier 
                          ...
                          : "Establishment identified. Vinea Managed Cloud node activated for your tier.")}
                  </p>
                </div>
                <button 
